...
const MAX_INPUT_LEN: usize = 768;
const MAX_QUERY_LEN: usize = 512;
const MAX_TARGET_LEN: usize = 192;
...
fn target_fixture() -> &'static TargetFixture {
...
fn split_query_and_target(input: &str) -> (&str, Option<&str>) {
    let split_at = input
        .char_indices()
...
fn query_is_too_large(query_text: &str, query: &QueryMol) -> bool {
...
fn selected_target_ids(data: &[u8], target_count: usize) -> [usize; TARGETS_PER_INPUT] {
    let seed = data
        .iter()
        .fold(0usize, |acc, byte| acc.wrapping_mul(167).wrapping_add(usize::from(*byte)));
...
fuzz_target!(|data: &[u8]| {
    ...
    if data.len() <= MAX_INPUT_LEN {
        ...
        evaluate_smarts_input(input.as_ref(), data);
    }

    let mut cursor = ByteCursor::new(data);
    let projected = synthesize_ga_like_query(&mut cursor);
    evaluate_smarts_input(&projected, data);
});
