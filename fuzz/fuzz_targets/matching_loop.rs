...
fn query_is_too_large(query: &smarts_rs::QueryMol) -> bool {
    query.atom_count() > 24 || query.bond_count() > 36 || query.component_count() > 8
}

fn query_is_too_generic(query: &smarts_rs::QueryMol) -> bool {
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(target) = target_text.as_ref().parse::<Smiles>() else {
        let bool_result = query.matches(target_text.as_ref());
        let count_result = query.match_count(target_text.as_ref());
        let materialized_result = query.substructure_matches(target_text.as_ref());
        ...
        match (&bool_result, &count_result, &materialized_result) {
            ...
            _ => panic!("matcher methods must agree on invalid target errors"),
        }
        return;
    };
    ...
    match (&bool_result, &count_result, &materialized_result) {
        ...
        _ => panic!("matcher methods must agree on success vs error"),
    }
    ...
    match CompiledQuery::new(query.clone()) {
        Ok(compiled) => {
            let prepared_bool =
                query.matches_prepared(&prepared).expect("prepared API must match compiled API");
            let prepared_count = query.match_count_prepared(&prepared)
                .expect("prepared count API must match compiled API");
            let prepared_matches = query.substructure_matches_prepared(&prepared)
                .expect("prepared match collection API must match compiled API");

            assert_eq!(prepared_bool, compiled.matches(&prepared));
            assert_eq!(prepared_count, compiled.match_count(&prepared));
            assert_eq!(
                prepared_matches.as_ref(),
                compiled.substructure_matches(&prepared).as_ref()
            );
        ...
        Err(compilation_error) => {
            let prepared_bool = query.matches_prepared(&prepared)
