        ...
        |context, mapped_count| {
            ...
            for_each_mapping(
                ...
                &mut |query_to_target| {
                    ...
                    insert_unique_match(&mut matches, canonical_match_key(&mapping), mapping);
                    false
                },
            );
            matches
...
fn two_atom_query_substructure_matches(
    ...
    for_each_two_atom_mapping(query, target, recursive_cache, initial_mapping, |mapping| {
        ...
        insert_unique_match(&mut matches, canonical_match_key(&ordered), ordered);
        false
    });
    matches
        .into_values()
...
fn three_atom_query_substructure_matches(
    ...
    for_each_three_atom_mapping(
        ...
        |mapping| {
            ...
            insert_unique_match(
                &mut matches,
                three_atom_canonical_mapping_key(mapping),
                ordered,
            );
            false
        },
...
fn three_atom_query_match_count(
    ...
    for_each_three_atom_mapping(
        ...
        |mapping| {
            ...
            matches.push(three_atom_canonical_mapping_key(mapping));
            false
        },
    );
    unique_match_count(matches)
...
fn insert_unique_match(matches: &mut UniqueMatches, key: Box<[usize]>, mapping: Box<[usize]>) {
    matches
        .entry(key)
        .and_modify(|existing| {
            if mapping < *existing {
...
fn unique_match_count(mut matches: MatchKeys) -> usize {
    matches.sort_unstable();
    matches.dedup();
    matches.len()
}

fn three_atom_canonical_mapping_key(mapping: &[Option<usize>]) -> Box<[usize]> {
    three_atom_canonical_key(
