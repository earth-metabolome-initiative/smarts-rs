//! Error types for SMARTS matching.

use thiserror::Error;

/// Errors raised by SMARTS matching.
#[derive(Debug, Error)]
pub enum SmartsMatchError {
    ...
    #[error("unsupported SMARTS atom primitive in current matcher implementation: {primitive}")]
    UnsupportedAtomPrimitive {
        /// Human-readable primitive name.
        primitive: &'static str,
    },
    /// The query uses a bond primitive that is not implemented yet.
    #[error("unsupported SMARTS bond primitive in current matcher implementation: {primitive}")]
