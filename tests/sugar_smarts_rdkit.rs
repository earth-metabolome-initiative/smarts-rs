...
fn sugar_smarts_1_to_4_match_rdkit_reference_panel() {
    ...
    for (query_name, query_smarts) in SUGAR_SMARTS.into_iter().take(4) {
        ...
        for (target_name, target_smiles) in TARGETS {
            ...
            let matched = query.matches(target_smiles).unwrap_or_else(|error| {
                panic!(
                    "{query_name} should stay inside the current matcher implementation against {target_name}: {error}"
                )
            });
            assert_eq!(
