...
fn aromaticity_edge_cases_should_agree_with_rdkit() {
    ...
    for case in CASES {
        ...
        let aromatic_matches = aromatic_query.substructure_matches(&target);
        assert_eq!(
        ...
        assert_eq!(
            aromatic_query.match_count(&target),
            case.aromatic_matches.len(),
            "{}: aromatic atom match count differs",
            case.name
        );

        let carbon_nitrogen_matches = carbon_nitrogen_query.substructure_matches(&target);
        assert_eq!(
        ...
        assert_eq!(
            carbon_nitrogen_query.match_count(&target),
